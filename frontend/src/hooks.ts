...
import { tradingAPI, authAPI, aiAPI, paymentAPI, statsAPI } from "./api";
...
export const useLastSignal = (symbol) => {
  ...
  const fetch = useCallback(async () => {
    ...
    try {
      ...
      // Set error state - no demo fallback
      setError(err.message || "Erro ao buscar sinal");
      return null;
    } finally {
      setLoading(false);
    }
...
export const useSignalStats = () => {
  ...
  const fetch = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await statsAPI.getSignalStats();
  ...
  return { fetch, loading, error, stats, setStats };
};
...
export const useAllSignals = () => {
  ...
  const fetch = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await statsAPI.getAllSignals();
  ...
  return { fetch, loading, error, signals, setSignals };
};
